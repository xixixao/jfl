...
Cl.equals = function equals(first, ...rest) {
  const isArray = Ar.isArray(first);
  const isSet = St.isSet(first);
  const isMap = Mp.isMap(first);

  for (const compared of rest) {
    if (isArray && !Ar.isArray(compared) || isSet && !St.isSet(compared) || isMap && !Mp.isMap(compared)) {
      return false;
    }
  }

  const args = [first, ...rest];
  return isArray ? Ar.equals(...args) : isMap ? Mp.equals(...args) : St.equals(...args);
...
Cl.equalsNested = function equalsNested(first, ...rest) {
  const isArray = Ar.isArray(first);
  const isSet = St.isSet(first);
  const isMap = Mp.isMap(first);

  for (const compared of rest) {
    if (isArray && !Ar.isArray(compared) || isSet && !St.isSet(compared) || isMap && !Mp.isMap(compared) || !isArray && !isSet && !isMap && compared !== first) {
      return false;
    }
  }

  const args = [first, ...rest];
  return isArray ? Ar.equalsNested(...args) : isMap ? Mp.equalsNested(...args) : isSet ? St.equalsNested(...args) : true;
...
Cl.isEmpty = function isEmpty(collection) {
...
Cl.count = function count(collection) {
...
Cl.contains = function contains(collection, value) {
...
Cl.containsKey = function containsKey(collection, key) {
...
Cl.any = function any(collection, predicateFn) {
...
Cl.every = function every(collection, predicateFn) {
  for (const [key, item] of collection.entries()) {
    if (!predicateFn(item, key, collection)) {
      return false;
    }
...
Cl.find = function find(collection, predicateFn) {
  for (const item of collection.values()) {
    if (predicateFn(item)) {
      return item;
    }
...
Cl.findX = function findX(collection, predicateFn) {
  for (const item of collection.values()) {
    if (predicateFn(item)) {
      return item;
    }
  ...
  throw new Error("Expected to find a value in collection matching given predicateFn, but didn't find one.");
...
Cl.findKey = function findKey(collection, predicateFn) {
  for (const [key, item] of collection.entries()) {
...
Cl.findKeyX = function findKeyX(collection, predicateFn) {
  for (const [key, item] of collection.entries()) {
    if (predicateFn(item, key)) {
      return key;
    }
  ...
  throw new Error("Expected to find a key in collection matching given predicateFn, but didn't find one.");
...
Cl.first = function first(collection) {
  for (const item of collection.values()) {
    return item;
  }
...
Cl.firstX = function firstX(collection) {
  for (const item of collection.values()) {
    return item;
  }
...
Cl.onlyX = function onlyX(collection) {
  let result = null;
  let foundFirst = false;

  for (const item of collection.values()) {
    if (foundFirst) {
      throw new Error('Expected exactly one item in collection, but there were more');
    }
  ...
  if (!foundFirst) {
    throw new Error('Expected exactly one item in collection, but the collection was empty.');
  }
...
Cl.last = function last(collection) {
  if (Array.isArray(collection)) {
...
Cl.lastX = function lastX(collection) {
...
Cl.at = function at(collection, index) {
  if (Array.isArray(collection)) {
...
Cl.atX = function atX(collection, index) {
  if (Cl.isEmpty(collection)) {
    throw new Error('Expected a non-empty collection, was empty instead.');
  }
...
Cl.firstKey = function firstKey(collection) {
...
Cl.firstKeyX = function firstKeyX(collection) {
  for (const key of collection.keys()) {
    return key;
  }
...
Cl.lastKey = function lastKey(collection) {
  let result = null;
...
Cl.lastKeyX = function lastKeyX(collection) {
  if (Cl.isEmpty(collection)) {
    throw new Error('Expected a non-empty collection, was empty instead.');
  }
...
Cl.forEach = function forEach(collection, fn) {
